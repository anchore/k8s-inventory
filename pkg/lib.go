...
func getBatchedInventoryReports(reports AccountRoutedReports, limits config.InventoryReportLimits) BatchedReports {
	...
	for account, accountReport := range reports {
		...
		for _, ns := range accountReport.Namespaces {
			...
			if (limits.PayloadBytes > 0 && state.currSize >= limits.PayloadBytes) ||
	...
	log.Infof("Finished batching %d inventory reports (threshold = %d namespaces, %d bytes)", batchCount, limits.Namespaces, limits.PayloadBytes)
	return batched
}
