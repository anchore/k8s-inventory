...
const healthDataType = "anchore_k8s_inventory_agent"
const healthReportAPIPathV2 = "v2/system/integrations/{{id}}/health-report"

type HealthReport struct {
	Id                   string          `json:"id,omitempty"`                     // uuid for this health report
	...
	IntegrationId        string          `json:"integration_id,omitempty"`         // id the integration instance
	...
	HealthData           HealthData      `json:"health_data,omitempty,omitempty"`  // K8s-inventory agent specific health data
}
...
type InventoryReportInfo struct {
	...
	Account             string      `json:"account"`               // Account to which the inventory report belongs
...
func PeriodicallySendHealthReportsGated(cfg *config.Application, integration *intg.Integration, gatedReportInfo *GatedReportInfo) {
	ticker := time.NewTicker(time.Duration(cfg.HealthPingIntervalSeconds) * time.Second)

	for {
		log.Infof("Waiting %d seconds to send health report...", cfg.HealthPingIntervalSeconds)

		healthReportId := uuid.New().String()
		lastReports := GetAccountReportInfoNoBlocking(gatedReportInfo, cfg)

		integration.Uptime = jstime.Now().UTC().Sub(integration.StartedAt)
		healthReport := HealthReport{
			Id:              healthReportId,
			ProtocolVersion: healthProtocolVersion,
			Timestamp:       jstime.Now().UTC(),
			...
			HealthReportInterval: cfg.HealthPingIntervalSeconds,
		}

		log.Infof("Sending health report (id:%s) covering %d accounts", healthReport.Id, len(healthReport.HealthData.AccountK8sInventoryReports))
		requestBody, err := json.Marshal(healthReport)
		if err != nil {
			log.Errorf("failed to serialize integration registration as JSON: %v", err)
		} else {
			log.Debugf("Size of healthreport as marshalled json %d bytes", len(requestBody)) // Bob. End of Remove this
			_, err = anchore.Put(requestBody, integration.Id, healthReportAPIPathV2, cfg.AnchoreDetails, "health report")
			if err != nil {
...
func GetAccountReportInfoNoBlocking(gatedReportInfo *GatedReportInfo, cfg *config.Application) AccountK8SInventoryReports {
	...
	if locked == true {
		...
		now := time.Now().UTC()
		inactiveAge := 200000 * float64(cfg.PollingIntervalSeconds)
		...
		for account, reportInfo := range gatedReportInfo.AccountInventoryReports {
			for _, batchInfo := range reportInfo.Batches {
				...
				} else if now.Sub(reportTime).Seconds() > inactiveAge {
...
func SetReportInfoNoBlocking(account string, count int, reportInfo InventoryReportInfo, gatedReportInfo *GatedReportInfo) {
	log.Debugf("Setting report (%s) for account '%s': %d/%d %s %s", reportInfo.ReportTimestamp, account,
	...
	if locked == true {
		defer gatedReportInfo.AccessGate.Unlock()
		gatedReportInfo.AccountInventoryReports[account] = reportInfo
	} else {
		// we prioritize no blocking over actually bookkeeping info for every sent inventory report
		log.Debugf("Unable to obtain mutex lock to include inventory report timestamped %s for %s: %d/%d %s %s in health report. Continuing.",
			reportInfo.ReportTimestamp, account, reportInfo.Batches[count].BatchIndex, reportInfo.BatchSize,
